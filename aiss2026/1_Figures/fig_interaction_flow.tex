...
\begin{tikzpicture}[
    ...
    % Lifelines
    \draw[thick, tclightgrey] (user.south) -- ++(0, -9.0);
    \draw[thick, tclightgrey] (orch.south) -- ++(0, -9.0);
    \draw[thick, tclightgrey] (rec.south) -- ++(0, -9.0);
    \draw[thick, tclightgrey] (ctx.south) -- ++(0, -9.0);
    ...
    \node[font=\scriptsize, text=tcgreen, align=center] at (3.2, -6.3) {Synthesise\\+ build trace};

    % 8. Orchestrator → User
    \draw[rmsg, color=tcblue] (3.2, -7.8) -- node[above, font=\scriptsize, align=center] {``HRV 12\% below baseline\ldots\\14--17h rain-free. Light session.''} (0, -7.8);
    \node[timelabel] at (-1.3, -7.8) {t\textsubscript{3}};
