...
\begin{tikzpicture}[
    ...
    % MCP Server layer (cyan) --- same chain principle: fixed gaps between
    % neighbours instead of independent offsets from a shared reference.
    \node[serverbox, below=1.5cm of recovery] (garmin) {Garmin\\:8104};
    \node[serverbox, right=0.4cm of garmin] (strava) {Strava\\:8103};
    \node[serverbox, below=1.5cm of context, xshift=-1.0cm] (weather) {Weather\\:8101};
    \node[serverbox, right=0.4cm of weather] (calendar) {Calendar\\:8105};
