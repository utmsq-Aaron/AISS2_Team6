...
Table~\ref{tab:competitors} in Appendix~\ref{sec:appendix:comparisons} surveys eleven platforms athletes currently use to manage training data -- from activity trackers (Strava, Garmin Connect) through conversational AI coaches (WHOOP Coach, Oura Advisor) and plan engines (TrainingPeaks, Athletica.ai, Runna, TriDot) to general-purpose LLMs. Its key finding: no existing platform simultaneously provides multi-source integration, cross-domain reasoning, and conversational interaction -- each covers at most one of the three.
