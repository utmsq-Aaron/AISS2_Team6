...
Table~\ref{tab:competitors} in Appendix~\ref{sec:appendix:comparisons} compares eleven platforms athletes currently use to manage training data. It includes activity trackers (Strava, Garmin Connect),  conversational AI coaches (WHOOP Coach, Oura Advisor), planning engines (TrainingPeaks, Athletica.ai, Runna, TriDot) and general-purpose LLMs as possible solutions. Its key finding: no existing platform simultaneously provides multi-source integration, cross-domain reasoning, and conversational interaction. Most of the time, each solution only covers one of the three.
