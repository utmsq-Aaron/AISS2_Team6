...
<div id="controls">
  ...
  <div class="ctrl-sep"></div>
  <button class="ctrl-btn" id="export-btn">&#11015; Export MP4</button>
</div>
...
// ── Manual export ─────────────────────────────────────────────────────────────
// Click to encode an MP4 in-browser (WebCodecs) at the injected orientation /
// resolution and download it — no server-side render. The full-screen status
// overlay covers the controls while encoding, so a click-to-cancel isn't exposed
// here; the encode completes on its own (or the user closes the flythrough).
...
_exportBtn.addEventListener('click', async () => {
  if (typeof VideoEncoder === 'undefined' || typeof Mp4Muxer === 'undefined') {
    _exportBtn.textContent = '✗ Not supported in this browser';
    setTimeout(() => { _exportBtn.textContent = '⬇ Export MP4'; }, 2800);
    return;
  }
  if (isEncoding) return;
  _exportBtn.textContent = '⏳ Encoding…';
  try { await exportFull(); } finally { _exportBtn.textContent = '⬇ Export MP4'; }
});
