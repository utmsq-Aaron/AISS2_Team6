...
On top of the multi-agent engine, the assistant behaves as a **personal coach**: concise and direct, it challenges the user rather than only pleasing them, stays anchored to a persistent goal across every chat, and is **proactive** — it schedules its own future check-ins.

- **Persona + goal injection.** The coach voice lives in `agents/prompts.py` `_base()` (shared by every agent); the user-facing challenge concentrates in the orchestrator's SYNTHESIS. The reworded permission rule keeps "act on data without asking permission" while explicitly allowing pointed *coaching* questions. A structured goal is injected into **every** turn (web + Telegram) via `core/user_memory.context_block()` → `goal_block()`, so goal + personality feel consistent across chats. There is **no separate injection site** — extending `context_block` covers both callers.
- **Structured goal store.** `core/goal_store.py` — one active goal per user at `data/user_memory/<slug>/goal.json` (beside `soul.md`), authored by BOTH a Settings/Dashboard form (`api/routers/goals.py`, `source="form"`) AND the coach in chat (the orchestrator's `upsert_goal` tool, `source="coach"`). `goal_progress()` derives the current value from live Strava/Garmin via the ToolHost (best-effort → `status:"unknown"`). The React Dashboard is rebuilt around this goal (progress ring); the dense analytics moved to the Analysis page.
- **Proactivity (durable, cross-chat, dedup'd).** The coach schedules its own re-activation with the `schedule_followup` tool → `core/schedule_store.py` (`data/schedules.json` + a `data/schedule_fired.json` fire-once log keyed by `email|reason_key|minute-slot`; write-time dedup by `reason_key`; recurrence re-arm). The **durable poll loop lives in `telegram_bridge.py`** (`_scheduler_loop`, the only always-on process — FastAPI runs `--reload` and can't host it): each tick it fires due wake-ups (composed live by re-running the note through the orchestrator, **coalesced** so the user is pinged once), drains the deep-report outbox, and hourly auto-schedules pre/post-calendar-event nudges + a weekly goal check-in. `dev_stack.sh` now starts the bridge; **proactivity is paused when the bridge is down.**
...
**`servers/google_maps_mcp.py`** is the second use of that template: it wraps the upstream `@modelcontextprotocol/server-google-maps` (stdio, launched via `npx -y`) — one persistent upstream session re-exposed over Streamable HTTP, registered in `MCP_SERVERS` (port 8108) and scoped to the `route` specialist (`AGENT_MCP_SCOPE`). It needs Node.js/`npx` on PATH and `GOOGLE_MAPS_API_KEY`. Heads-up: that upstream package is npm-deprecated (0.6.2) and calls Google's legacy Places API — fine for now, but the candidate to replace with a native FastMCP server against *Places API (New)* later.
