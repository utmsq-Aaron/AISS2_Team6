...
- **Persona + goal injection.** The coach voice lives in `agents/prompts.py` `_base()` (shared by every agent); the user-facing challenge concentrates in the orchestrator's SYNTHESIS. The reworded permission rule keeps "act on data without asking permission" while explicitly allowing pointed *coaching* questions. All of a user's active goals are injected into **every** turn (web + Telegram) via `core/user_memory.context_block()` → `goal_block()`, so goal + personality feel consistent across chats. There is **no separate injection site** — extending `context_block` covers both callers.
